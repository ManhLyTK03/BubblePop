...
public class mapRandom : MonoBehaviour
{
    ...
    void Update(){
        if(checkWin){
            // Kiểm tra xem có GameObject nào có tag "ballMap" không
            GameObject ballMapObject = GameObject.FindWithTag("ballMap");

            if(ballMapObject == null){
        ...
        for (int i = 0; i <= col ; i++)
        {
            for (int j = 0; j <= row ; j++)
            {
                if(i%2 != 1 || j != row){
                    ...
                    newBallMap.transform.GetChild(0).GetComponent<SpriteRenderer>().sprite = ballColors[Random.Range(0, ballColors.Length)];
