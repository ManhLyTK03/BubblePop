    ...
    void OnTriggerEnter2D(Collider2D other){
        if (other.gameObject.tag == "ballMap" && !isArrange)
        {
            ...
            if(transform.position.y > other.transform.position.y){
                transform.position = new Vector3(transform.position.x, other.transform.position.y + distanceY, transform.position.z);
            }
            if(transform.position.y < other.transform.position.y){
                transform.position = new Vector3(transform.position.x, other.transform.position.y - distanceY, transform.position.z);
            }
            if(transform.position.x > other.transform.position.x){
                transform.position = new Vector3(other.transform.position.x + distanceX, transform.position.y, transform.position.z);
            }
            else{
                transform.position = new Vector3(other.transform.position.x - distanceX, transform.position.y, transform.position.z);
            }
